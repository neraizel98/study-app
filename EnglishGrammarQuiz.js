(function () {
    ...
    const banks = {
        ...
        e3: [Q('He ___ soccer every day.', 'plays', ['play', 'played', 'is playing'], '3인칭 단수 현재에는 -(e)s를 붙입니다.'), Q('Does Mina ___ milk?', 'like', ['likes', 'liked', 'liking'], 'does 뒤에는 동사원형이 옵니다.'), Q('I ___ not know him.', 'do', ['does', 'am', 'is'], 'I의 일반동사 부정문에는 do를 씁니다.')],
        ...
        e6: [Q('He runs ___.', 'quickly', ['quick', 'quicker', 'quickness'], '동사 runs를 꾸미므로 부사가 필요합니다.'), Q('Monday 앞에 쓰는 전치사는?', 'on', ['at', 'in', 'under'], '요일 앞에는 on을 씁니다.'), Q('이유를 연결하는 접속사는?', 'because', ['but', 'or', 'under'], 'because는 이유를 나타냅니다.')],
        ...
        m2: [Q('She ___ now.', 'is studying', ['studied', 'has studied', 'studying'], '현재 진행은 be + -ing입니다.'), Q('go의 과거형은?', 'went', ['goed', 'gone', 'goes'], 'go는 불규칙동사입니다.'), Q('tomorrow와 잘 어울리는 것은?', 'will + 동사원형', ['동사의 과거형', 'had + p.p.', 'was + -ing'], '앞으로의 일은 will + 동사원형으로 나타낼 수 있습니다.')],
        ...
        m4: [Q('I enjoy ___ books.', 'reading', ['to read', 'read', 'to reading'], 'enjoy의 목적어로 동명사를 씁니다.'), Q('I came here ___ you.', 'to help', ['helping', 'helped', 'to helping'], '목적을 나타내는 to부정사입니다.'), Q('want 뒤에 알맞은 것은?', 'to + 동사원형', ['동명사', '과거분사', '전치사 + 동사원형'], 'want는 to부정사를 목적어로 취합니다.')],
        ...
        m6: [Q('셋 이상 중 가장 큰 것을 비교할 때는?', '최상급', ['비교급', '원급', '현재분사'], '셋 이상의 범위에서는 최상급을 씁니다.'), Q('이유를 나타내는 접속사는?', 'because', ['but', 'or', 'on'], 'because는 이유를 나타내는 절을 연결합니다.'), Q('Monday 앞에 알맞은 전치사는?', 'on', ['at', 'in', 'from'], '요일 앞에는 on을 씁니다.')],
    ...
    function makeCorrection(sentence) {
        const rules = [
            [/^(He|She|Mina|Jin|Sora|Tom|Yuna) ([A-Za-z]+)s(\b.*)$/i, (_all, subject, verb, rest) => `${subject} ${verb.toLowerCase() === 'i' ? 'are' : verb}${rest}`],
            [/^I am\b/i, 'I is'],
            [/^They are\b/i, 'They is'],
            [/^There are\b/i, 'There is'],
            [/^Let[’']s ([A-Za-z]+)(\b.*)$/i, (_all, verb, rest) => `Let’s ${verb}ing${rest}`]
        ];
