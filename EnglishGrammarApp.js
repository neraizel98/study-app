(function () {
    ...
    const principleDetails = {
        ...
        e3: '일반동사는 행동·습관뿐 아니라 know/like처럼 상태도 나타냅니다. 현재형에서는 주어가 he·she·it처럼 한 사람이나 한 사물일 때만 동사에 -(e)s가 붙습니다. 의문문이나 부정문에서 does를 사용하면 3인칭 단수 표시는 does가 담당하므로 본동사는 반드시 원형으로 돌아갑니다.',
        ...
        e5: '명령문과 조동사는 짧지만 말하는 사람의 의도와 태도를 분명하게 전달합니다. 명령문은 듣는 사람 you를 생략하고 동사원형으로 시작하며, 조동사는 능력·의무·허가 같은 의미를 본동사에 더합니다. can/must 같은 조동사 뒤에는 주어에 관계없이 동사원형을 씁니다.',
        ...
        m6: '비교 표현은 대상의 수와 비교 기준에 따라 원급·비교급·최상급을 선택합니다. 접속사와 전치사는 모두 말을 연결하지만 and/but/or 같은 접속사는 단어·구·절을 연결하고, 전치사는 명사(구)와 다른 성분의 관계를 나타냅니다. 빈칸 뒤에 주어와 동사가 있는지 확인하면 두 품사를 구별하기 쉽습니다.',
        m7: '의문사가 목적어나 부사어이면 그 뒤에 의문문 어순을 씁니다. 의문사가 주어이면 Who called you?처럼 바로 동사를 씁니다. 의문사만 붙이고 평서문 어순을 그대로 두는 실수가 많으므로 be동사인지 일반동사인지 먼저 구별하세요. 부가의문문은 앞 문장의 동사와 주어를 그대로 받아 긍정과 부정을 반대로 만드는 구조입니다.',
